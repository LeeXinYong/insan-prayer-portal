<button {{ isset($id) ? "id=$id" : "" }} type="{{ $type ?? "button" }}" class="btn {{ $color ?? "btn-custom-light btn-active-custom-light" }} {{ $size ?? "min-w-60px min-w-lg-150px min-h-40px" }} {{ ($round_button ?? false) ? "rounded-6" : "rounded-1" }} {{ $classes ?? "" }}" {!! $attributes ?? "" !!} {{ ($disabled ?? false) ? "disabled" : "" }}>
