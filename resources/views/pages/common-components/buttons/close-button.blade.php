@if($indicator ?? false)
    @include("pages.common-components.buttons.indicator-buttons.hover-indicator-button", [
        ...
        "label" => $label ?? __("general.button.close"),
        "label_classes" => $label_classes ?? (($icon_only ?? config("layout.button_icon_enabled")) && is_string($icon ?? "fs-3 ra-cancel") && !empty($icon ?? "fs-3 ra-add") ? "d-none d-lg-inline" : null),
        "message" => $message ?? __("general.button.closing"),
        "message_classes" => $message_classes ?? (($icon_only ?? config("layout.button_icon_enabled")) && is_string($icon ?? "fs-3 ra-cancel") && !empty($icon ?? "fs-3 ra-add") ? "d-none d-lg-inline" : null)
    ])
@elseif(isset($link))
