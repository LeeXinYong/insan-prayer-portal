<a {{ isset($id) ? "id=$id" : "" }} {{ isset($link) ? "href=$link" : "" }} {{ isset($target) ? "target=$target" : "" }} class="btn {{ $color ?? "btn-custom-light btn-active-custom-light" }} {{ $size ?? "min-w-60px min-w-lg-150px min-h-40px" }} {{ ($round_button ?? false) ? "rounded-6" : "rounded-1" }} {{ ($disabled ?? false) ? "link-button-disabled" : "" }} {{ $classes ?? "" }}" {!! $attributes ?? "" !!}>
