...
function getDemos(pathDemos) {
    ...
    if (demos.length === 0) {
        demos = [process.env.LAYOUT_DEMO || 'demo1'];
    }
    return demos;
}
...
/// Customize
mix.copyDirectory('resources/customize', `public/${demo}/customize`);

(glob.sync('resources/sass/*.+(scss|sass)') || []).forEach(file => {
    mix.sass(file, file.replace(/resources\/sass/, `public/${demo}/css`).replace(/\.scss$/, '.css'));
});
