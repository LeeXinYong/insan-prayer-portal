...
{
    public const TABLE_NAME = "credential_table";
    /**
    ...
    public function dataTable($query): DataTableAbstract
    {
        return datatables()
            ->eloquent($query)            
            ->rawColumns(["consumer_id", "signature"])
            ->addColumn("action", function (Credential $model) {
                $actions = [
                    "edit" => [
                        "url" => route("credential.edit", ["credential" => $model->consumer_id]),
                        "label" => __("general.button.edit"),
                        "disabled" => Auth::user()->cannotUpdate(Credential::class)
                    ],
                    "delete" => [
                        "url" => route("credential.destroy", ["credential" => $model->consumer_id]),
                        "label" => __("general.button.delete"),
                        "disabled" => Auth::user()->cannotDelete(Credential::class)
                    ]
                ];
     ...
     *
     * @param \App\Models\CredentialsDataTable $model
     * @return \Illuminate\Database\Eloquent\Builder
     */
    public function query(Credential $model): Builder
    {
        return $model->newQuery()->select('consumer_id', 'signature');
    }
    ...
    {
        return [
            Column::make('consumer_id')
                ->title( __("credential.form_label.consumer_id") )
                ->name('credentials.consumer_id')
                ->orderable(true),
            Column::make('signature')
                ->title( __("credential.form_label.signature") )
