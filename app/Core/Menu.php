...
class Menu
{
    ...
    private function _generateItemLink($item)
    {
        ...
        if ($this->_matchItemByActivePath($item) === true) {
            $classes[] = "active";
        }
    ...
    private function _generateItemContent($item)
    {
        ...
        if (isset($item["content"]) && (isset($item["content"]["classes"]) || isset($item["content"]["label"]))) {
            echo "<div " . Util::getHtmlClass($classes) . ">";
            echo "<span class='" . ($item["content"]["classes"] ?? "") . "'>" . __($item["content"]["label"]) . "</span>";
            echo "</div>";
        }
    }
    ...
    private function _matchItemByPath($item): bool
    {
        if(isset($item["path"]) && Route::has($item["path"])) {
            $path_pattern = str_replace(url("") . "/", "", route($item["path"], $item["parameters"] ?? []));
            if(count(explode("/", $this->path)) == count(explode("/", $path_pattern)) && (preg_match("/^" . str_replace("/", "\/", $path_pattern) . "(?=$|\s)/", $this->path))) {
                return true;
            }
        }
        return false;
    ...
    private function _matchItemByActivePath($item): bool
    {
        $wildcard = (isset($item["active_path"]) && count($item["active_path"]) <= 1 && ($item["wildcard"] ?? true)) ? "*" : "";
        return isset($item["active_path"]) && Request::is(
            array_map(function ($path) use ($wildcard) {
                $parameters = [];
                if (is_array($path)) {
                    $parameters = $path["parameters"] ?? [];
                    $path = $path["path"];
                }
                return Route::has($path) ? (str_replace(url("") . "/", "", route($path, $parameters)) . $wildcard) : ($path == "" ? "/" : "");
            }, $item["active_path"])
        );
    }
    ...
    private function _buildBreadcrumb($items, &$breadcrumb, $options, $level = 0)
    {
        ...
        foreach ($items as $item) {
            ...
            if (isset($item["breadcrumb-title"])) {
                $title = __($item["breadcrumb-title"]);
                ...
                $title = __($item["heading"]);
            }

            if ($this->_matchItemByPath($item)) {
